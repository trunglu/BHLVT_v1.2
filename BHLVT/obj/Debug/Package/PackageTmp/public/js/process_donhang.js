    ...
    for (var i = 0; i < colCount; i++) {
        ...
        } else if (i == 2)
        {
            ...
            newcell.innerHTML = '<div class="hidden"><input id="cctenhang' + (rowCount - 1) + '" name="cctenhang[' + (rowCount - 1) + ']" value="Tên Hàng" style="height:30px;"></div>' +
                ' <input id="scctenhang' + (rowCount - 1) + '" name="scctenhang' + (rowCount - 1) + '" class="form-control input-sm" placeholder="Tên Hàng" style="width:200px" />';
            initComboTenHang('cctenhang' + (rowCount - 1));
            oninitHangHoa((rowCount - 1));
            
        } else if (i == 4) {
            newcell.innerHTML = '<input type="hidden" id="namedvt' + (rowCount - 1) + '" name="namedvt[' + (rowCount - 1) + ']" value=""/><input id="ccdvt' + (rowCount - 1) + '" name="ccdvt[' + (rowCount - 1) + ']" value="Đơn VT" style="width: 80px;height: 22px;">';
         ...
         newcell.innerHTML = ' <input type="checkbox" class="check-box-km" name="check_km' + (rowCount - 1) + '" value="' + (rowCount - 1) + '" style="width: 50px;" id="check_km' + (rowCount - 1) + '"/>';
            ...
            newcell.innerHTML = ' <input type="text" class="form-control input-sm" maxlength="6" size="6" value="0" onblur="onChangeSoLuong(soluong' + (rowCount - 1) + ',' + (rowCount - 1) + ')" onkeyup="onChangeSoLuong(soluong' + (rowCount - 1) + ',' + (rowCount - 1) + ')" name="soluong[' + (rowCount - 1) + ']" id="soluong' + (rowCount - 1) + '" onfocus="select_value(' + (rowCount - 1) + ')" style="text-align:center; width:70px;height: 22px"/>';
            initNumberSoluong('soluong' + (rowCount - 1), (rowCount - 1));

           
        } else if (i == 7) {
            newcell.innerHTML = ' <input type="text" class="form-control input-sm" onkeyup="onChangeGia(dongia' + (rowCount - 1) + ',' + (rowCount - 1) + ')" value="0" name="dongia[' + (rowCount - 1) + ']" id="dongia' + (rowCount - 1) + '" style="text-align:right; width:150px;"/>';
        } else if (i == 8) {
            newcell.innerHTML = ' <input type="hidden" value="1" id="flagdiscount' + (rowCount - 1) + '"/> <input onblur="changeCK(chietkhau' + (rowCount - 1) + ',' + (rowCount - 1) + ')" onclick="changeCK(chietkhau' + (rowCount - 1) + ',' + (rowCount - 1) + ')" onkeyup="changeCK(chietkhau' + (rowCount - 1) + ',' + (rowCount - 1) + ')" name="chietkhau[' + (rowCount - 1) + ']" id="chietkhau' + (rowCount - 1) + '" value="0" style="text-align:right;height: 22px"/>';
            initNumberCK('chietkhau' + (rowCount - 1), (rowCount - 1));

        } else if (i == 9) {
            newcell.innerHTML = ' <input readonly type="text" class="form-control input-sm"   name="tienck[' + (rowCount - 1) + ']" id="tienck' + (rowCount - 1) + '" value="0" onblur="onChangeFixedDiscount(tienck' + (rowCount - 1) + ',' + (rowCount - 1) + ')" style="text-align:right; width:100px;"/>';
            initTienCK('tienck' + (rowCount - 1), (rowCount - 1));
        } else if (i == 10) {
            newcell.innerHTML = '<input type="text" class="form-control thanhtoan input-sm" readonly name="thanhtoan[' + (rowCount - 1) + ']" id="thanhtoan' + (rowCount - 1) + '" value="0" style="text-align:right; width:100px;"/>';
        }
...
var checkSlTon = function () {
    ...
    var table = document.getElementById('dataTable');
    var rowCount = parseInt(document.getElementById('lrow').value+"");
   // console.log("rcount: "+(rowCount+1));
    var x = 1;
    $('#modal_loading').modal({ backdrop: true });
    for (i = 0; i < rowCount - 1; i++) {
        if (typeof $('#soluong' + i).val() != "undefined") {
            var slvao = $('#soluong' + i).val().replace(/,/g, '');
            var soluong = parseInt(slvao);
            console.log('CheckSLTon  :' + x + " " + soluong);
            onChangeSoLuong_1('', i);
            x++;
        }
        else {
...
function  SaveDH() {
    ...
    if (confirm('Bạn có muốn lưu đơn hàng ?')) {
        var table = document.getElementById('dataTable');
        var rowCount = document.getElementById('lrow');
...
var initNumberCK = function (id, index) {
    $('#chietkhau' + index).numberspinner({
        ...
        onChange: function (newval,oldval) {
            ...
            $('#chietkhau' + index).numberbox('setValue', newval);
            $('#chietkhau' + index).attr('value',newval);
            onChangeSoLuong(id, index);*/
        },
        onClickIcon: function () {
            ...
            $('#chietkhau' + index).numberbox('setValue', ck);
...
var initTienCK = function (id, index) {
    $('#tienck' + index).inputmask('decimal', { rightAlign: true, groupSeparator: ",", autoGroup:true });
...
function initNumberSoluong(id, index) {
    $('#soluong' + index).numberspinner({
        ...
        //editable: false,
        onChange: function (newval,oldval) {
            $('#soluong' + index).val(newval);
            //$('#soluong' + index).attr('value', newval);
            processFlagdiscount(index);
            console.log($('#chietkhau' + index).val());
            onChangeSoLuong(id, index);
        },
        events: {
            blur: function () {
